...
flowchart TD
    ...
    subgraph MCP["📦 MCP Wrapper Canister"]
        Auth["🔐 Authentication<br/>API Keys, OAuth"]
        Tools["🛠️ High-Level Tools<br/>swap, quote, claim"]
        Orch["⚙️ Orchestration<br/>Multi-step workflows"]

        Auth --> Tools
        Tools --> Orch
    end

    MCP -->|Candid Interface| Backend["🏦 Your Backend Canister<br/>(DEX, Ledger, Custom Logic)"]
...
```motoko
// ✅ CORRECT - Check that caller is the authorized owner
if (ownerPrincipal != context.owner) {
  return makeError("Unauthorized");
};

// ❌ WRONG - Allowing any authenticated user
...
```motoko
// ✅ CORRECT - Pull exact amount needed via allowance
...
// ❌ WRONG - Asking user to send funds first
...
```motoko
// ✅ CORRECT - Approvals should expire
expires_at = ?(Time.now() + 300_000_000_000); // 5 minutes

// ❌ WRONG - Infinite approval
...
```motoko
// ✅ CORRECT - Pull, use, and return in same operation
// Pull funds → Approve → Execute → Return remainder

// ❌ WRONG - Storing user funds long-term
...
```motoko
// Check → Pull → Approve → Execute → Return
